...
//////
//////    // スケルトンのノード→ボーン番号マップへの参照を取っておく
//////    auto& bone_index_map = skeleton_.Data().bone_index_of_;
//////
//////    // シーン中の全ノードをチェック
//////    size_t ni;
...
//////    if (r < 1e-3f) r = 1.0f;
//////
//////    // FbxSkeleton 内の scene_radius_ に書く
...
//
//    // スケルトン構築（FbxSkeleton に委譲）
//    if (!skeleton_.BuildFromScene(scene)) {
//        return false;
//    }
//
//    // メッシュ展開（CPU 側）
//    ExpandAllNodes(scene);
//
//    // GPU バッファ作成
//    if (!CreateGpuBuffers()) {
//        return false;
...
bool UfbxStaticModel::Load(const char* fbx_path)
{
    ...
    // メッシュ展開（実装は UfbxStaticModel_Mesh.cpp にある）
    ExpandAllNodes(scene);
